"""
MineToday — Module Relevance Prediction (v27)
=============================================
Riwayat LB: v11 0.65725 | v21 0.65888 | v22 0.65670 | v23 0.65827 |
            v24 0.66080 | v25 0.65848 | v26 0.65950

BACA INI DULU -- angka yang menentukan strategi:
  Simpangan baku 7 skor LB di atas          = 0.00137
  Selisih model TERBESAR yang bisa diukur   = 0.0007  (25+ split pseudo-test)
  Adversarial AUC train-vs-test             = 0.4964  (TIDAK ada shift)
  Sebaran NDCG antar-seed model yang sama   = 0.00085
  => Naik-turunnya LB 0.6567..0.6608 didominasi sampling noise public LB
     (~31% test = ~310 user), BUKAN kualitas model.

Sebelas eksperimen, ~25 split pseudo-test. Semua yang diuji:
  ...
  pred_text_clf                           +0.0004 (7/10)     dipakai
  prasyarat berbasis MIN (dari katalog)   +0.0006 +-0.0006   dipakai
  regressor beregulasi sbg meta kedua     -- bagian v27      dipakai
  GABUNGAN keempatnya (v27)               +0.0005 +-0.0007   dipakai

Gabungan empat efek positif ternyata TIDAK menjumlah (+0.0005, bukan +0.002):
efeknya saling tumpang tindih. Jadi v27 adalah konfigurasi terbaik yang bisa
dipertanggungjawabkan, tapi selisihnya thd v24 tetap di bawah noise LB.

Yang BARU di v27 dibanding v26:
  [+] prereq_min : minimum skor prasyarat per modul, dibaca langsung dari kolom
      prerequisite_level katalog ("Butuh Python & Stat" = MIN dari keduanya).
      v24 memakai skill_match = RATA-RATA, padahal katalog menuntut semuanya
      terpenuhi. Terukur: P(M_016 relevan) naik 0.17 -> 0.67 mengikuti MIN;
      P(M_011) 0.12 -> 0.33.
  [+] all_skill_low : aturan eksplisit M_001 "Pemula (Semua Skor 0-1)".
      Terukur: M_001 relevan 49.5% bila semua skor <=1, vs 27.4% bila tidak.
  [+] pred_reg2 : LGBM kedua yg diregulasi (min_child_samples=60, reg_lambda=5,
      900 pohon) sebagai meta-feature tambahan.
  [-] meta-ranker tetap tidak dipakai (v27_blend = v27_ridge dalam pengukuran).

Blok PSEUDO-TEST bawaan membandingkan META v24 / v26 / v27 di 1000 user train
yang ditahan -- estimasi jujur, dan alat yang benar untuk menguji ide berikutnya.
...
SEEDS        = [42, 202, 777, 2026, 31337, 7, 123, 999]   # 8 seed (dari 5)
REPEAT_SEEDS = [42, 123, 2024, 7777]                      # 4 repeat CV
KNN_K        = 30
TEXT_ALPHA   = 3.0            # alpha terpilih di 18/20 fold saat v25 disweep
RUN_PSEUDOTEST = True         # blok validasi jujur; matikan kalau mau cepat
...
# [v27] PRASYARAT berbasis MIN, dibaca langsung dari kolom prerequisite_level
...
if RUN_PSEUDOTEST:
    ...
    print("Ini estimasi JUJUR utk performa di test -- CV cenderung optimistis.")
    allu=np.array(sorted(train_wide.user_id))
    rs_=np.random.RandomState(SEED); pm_=rs_.permutation(len(allu))
    ptest, devu = allu[pm_[:1000]], allu[pm_[1000:]]
    oof_pt=run_oof(devu,[SEED],[SEED])[0]
    full_pt=train_predict(devu,[ptest],SEEDS[:3])[0]
    for cols,nm in [(META_v24,"META v24     "),(META_v26,"META v26     "),(META,"META v27     ")]:
        ...
        print(f"  {nm}: NDCG@5 pseudo-test = {ndcg_of(p,'s'):.5f}  "
              f"(linear-gain {ndcg_of(p,'s',False):.5f})")
    print("="*66)
...
test_pred["pred"]=np.clip(ridge.predict(test_pred[META].fillna(0)),0,1)
submission=(test_pred.pivot(index="user_id",columns="module_id",values="pred")
...
out=OUT_DIR/"submission_v27.csv"; submission.to_csv(out,index=False)
