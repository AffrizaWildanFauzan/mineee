"""
=======================================================================
 MineToday v37 -- encoder kalimat PRETRAINED sbg uji, bukan klaim
=======================================================================
BACA INI DULU. Saya TIDAK bisa menguji jalur embedding: sandbox pengembangan
saya tidak punya torch/transformers dan tanpa internet. Kode ini ditulis
supaya RUN ANDA yang memutuskan lewat blok holdout tersegel. Ekspektasi saya
sendiri: ruangnya nyaris nol. Alasannya di bawah.

APA YANG DITAMBAHKAN
  pred_emb_clf  LogisticRegression di embedding kalimat -> modul dominan
                (analog langsung dari pred_text_clf, tapi semantik)
  pred_emb      Ridge di embedding -> 17 nilai relevansi
Encoder: transformer pretrained multibahasa, mean-pooling, dinormalkan.
Ditulis dgn `transformers` saja (bukan sentence-transformers) supaya lebih
portabel. Kalau model/torch tidak ada, JALUR INI DILEWATI dan run tetap
menghasilkan submission yang sah.

CARA MENGAKTIFKAN DI KAGGLE (tidak butuh internet)
  Add Input -> Models -> cari "paraphrase-multilingual-MiniLM-L12-v2"
  Alternatif Indonesia: indobenchmark/indobert-base-p1
  Nyalakan GPU (opsional; CPU juga jalan, ~3 menit untuk 5000 dokumen).
  Kalau nama foldernya lain, tambahkan ke daftar EMB_DIRS.

KENAPA EKSPEKTASI SAYA RENDAH (e65-e67, semuanya terukur)
  Saat user menyebut SATU modul, model sekarang sudah benar 96.1%.
  Saat menyebut beberapa, akurasi 41-69% -- terlihat seperti ruang, TAPI:
  user dgn chat IDENTIK punya modul rank-1 BERBEDA di 62-75% kasus.
  Encoder mana pun membaca input sama harus keluarkan output sama, jadi
  tembok itu bukan soal pemahaman bahasa.
  Model juga sudah jauh di atas tebak-acak-di-antara-yang-disebut:
    2 sebutan 68.8% vs 50%   3 sebutan 59.4% vs 33%
    4 sebutan 55.1% vs 25%   5 sebutan 46.0% vs 20%
  artinya disambiguasi sudah dikerjakan lewat asesmen.
  Satu sisa peluang: 1082 user yang modulnya TIDAK terdeteksi keyword saya
  (akurasi 13.9%) -- sebagian mungkin parafrase yang lolos regex. Di situlah
  embedding punya kesempatan, dan hanya di situ.

KELUARAN (empat, dari fit base yang sama)
  submission_v37_dua.csv    META v26 + listnet + embedding
  submission_v37_lnet.csv   META v26 + listnet            (= v35, LB 0.66113)
  submission_v37_emb.csv    META v26 + embedding
  submission_v37_v26.csv    META v26 murni                (pembanding)

CARA PAKAI: jalankan, BACA BLOK HOLDOUT TERSEGEL di awal output. Ia menilai
keempatnya dan menyebut pemenangnya. Ingat SE satu pengukuran di 1000 user
~ +-0.0015, jadi selisih di bawah itu jangan dianggap pasti -- listwise
pernah menang +0.0030 di sana dan memberi NOL di papan publik.

Runtime ~3 jam (CPU) + ~3 menit encode. Dgn GPU sedikit lebih cepat.
"""
...
# ------------------------------------------------------------- EMBEDDING
# [v37] Encoder kalimat pretrained sbg pengganti/pendamping TF-IDF.
# Kaggle tidak punya internet, jadi modelnya harus DILAMPIRKAN sbg dataset
# atau lewat menu "Add Model". Cari di beberapa lokasi yang lazim; kalau
# tidak ketemu atau torch tidak ada, seluruh jalur embedding DILEWATI dan
# run tetap menghasilkan submission yang sah.
#
# Cara melampirkan di Kaggle:
#   Add Input -> Models -> cari "paraphrase-multilingual-MiniLM-L12-v2"
#   (atau Datasets: sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2)
# Alternatif untuk bahasa Indonesia: indobenchmark/indobert-base-p1
EMB_DIRS = [
...
HAS_EMB=False; X_EMB=None
try:
    import torch
    from transformers import AutoTokenizer, AutoModel
    _mp=_cari_model()
    if _mp is None:
        print("EMBEDDING: model pretrained tidak ditemukan -> jalur embedding DILEWATI.")
        print("           (lampirkan model lewat Add Input -> Models untuk mengaktifkan)")
    else:
        print(f"EMBEDDING: memakai {_mp}",flush=True)
        _dev="cuda" if torch.cuda.is_available() else "cpu"
        _tok=AutoTokenizer.from_pretrained(str(_mp))
        _mdl=AutoModel.from_pretrained(str(_mp)).to(_dev).eval()
        _out=[]
        with torch.no_grad():
            for i in range(0,len(docs),EMB_BATCH):
                b=_tok(docs[i:i+EMB_BATCH],padding=True,truncation=True,
                       max_length=EMB_MAXLEN,return_tensors="pt").to(_dev)
                h=_mdl(**b).last_hidden_state
                m=b["attention_mask"].unsqueeze(-1).float()
                _out.append(((h*m).sum(1)/m.sum(1).clamp(min=1e-9)).cpu().numpy())
                if i % (EMB_BATCH*20)==0: print(f"  encode {i}/{len(docs)}",flush=True)
        X_EMB=normalize(np.vstack(_out).astype(np.float64))
        del _mdl
        if _dev=="cuda": torch.cuda.empty_cache()
        HAS_EMB=True
        print(f"EMBEDDING: matriks {X_EMB.shape[0]} user x {X_EMB.shape[1]} dim")
except Exception as e:
    print(f"EMBEDDING: gagal ({type(e).__name__}: {e}) -> jalur embedding DILEWATI.")
...
_E      =["pred_emb_clf","pred_emb"] if HAS_EMB else []
META_EM =META+_E                                                     # [v37] embedding
META_2  =META+["pred_lnet"]+_E                                       # [v37] keduanya
...
print("\nMenulis submission:")
s2=tulis(META_2,"v37_dua"); sL=tulis(META_LN,"v37_lnet")
sM=tulis(META_EM,"v37_emb"); sV=tulis(META,"v37_v26")
tp=lambda s: np.argsort(-s[MODULE_COLS].to_numpy(),1)[:,:5]
ident=lambda x,y: np.mean([set(p)==set(q) for p,q in zip(x,y)])
print(f"\n  v37_dua vs v37_v26 : top-5 set identik {ident(tp(s2),tp(sV)):.3f}")
print(f"  v37_dua vs v37_lnet: top-5 set identik {ident(tp(s2),tp(sL)):.3f}")
