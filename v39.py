"""
=======================================================================
 MineToday v39 — encoder IndoBERTweet (indolem/indobertweet-base-uncased)
=======================================================================
Sama persis dengan v38, HANYA encoder teksnya yang diganti:
    paraphrase-multilingual-MiniLM-L12-v2   ->   indolem/indobertweet-base-uncased

CATATAN TEKNIS YANG PENTING
  IndoBERTweet adalah BERT BIASA, bukan model sentence-transformers. Ia tidak
  pernah dilatih kontrastif untuk kemiripan kalimat. sentence-transformers akan
  otomatis membungkusnya dgn mean-pooling (log-nya akan bilang "Creating a new
  one with mean pooling") — itu normal dan memang yang kita mau.
  Akibatnya: `pred_emb_sim` (kosinus user vs katalog modul, TANPA training)
  kemungkinan besar LEBIH BURUK dari MiniLM, karena embedding BERT mentah
  punya geometri kosinus yang buruk (anisotropi). Dua sinyal lain dilatih
  ulang (LogReg/Ridge) jadi tidak terpengaruh masalah itu.

CARA MEMBANDINGKAN DENGAN RUN v38 ANDA
  Di bagian "NDCG@5 tiap sinyal sendirian", bandingkan langsung dgn v38:
      sinyal            MiniLM (v38)     IndoBERTweet (run ini)
      pred_emb_sim        0.46716              ?
      pred_emb_clf        0.60545              ?
      pred_emb            0.62167              ?
  Pembanding TF-IDF yang harus dikalahkan: pred_text 0.63784, pred_text_clf 0.61574.
  Dan lihat koefisien meta-Ridge di akhir: di v38, pred_emb_sim dan pred_emb
  dapat bobot 0.000 (dibuang Ridge). Kalau di sini juga 0.000, jawabannya sama.

CARA MENJALANKAN DI KAGGLE — pilih SATU
  (A) Settings -> Internet -> ON. Model diunduh otomatis (~500 MB, sekali).
      Kalau sentence-transformers belum ada, hapus komentar baris !pip di bawah.
  (B) Tanpa internet: Add Input -> Models -> cari "indobertweet".
      Kode mencari model lokal DULU sebelum mencoba unduh.
  Kalau dua-duanya gagal, jalur embedding DILEWATI — lihat peringatan di penutup.
  GPU opsional; CPU ~4-6 menit untuk 5017 dokumen (IndoBERTweet 768 dim,
  lebih besar dari MiniLM 384, jadi sedikit lebih lambat).

EKSPEKTASI SAYA (terukur, e78-e80 — silakan dibantah oleh run ini)
  7 representasi teks x 3 model diuji di OOF 4000 user: SEMUANYA mendarat di
  pita 0.62-0.64. Yang terbaik 0.63993 (TF-IDF karakter). MiniLM 0.62167 ada
  DI DALAM pita, di bawah median.
  Petunjuk terkuat: hitungan kata mentah 322 dimensi dapat 0.63979 — praktis
  seri dgn yang terbaik. Artinya sinyal teks di sini pada dasarnya "kata kunci
  modul mana yang muncul": masalah PENCOCOKAN LEKSIKAL, bukan pemahaman makna.
  Keunggulan IndoBERTweet (Indonesia informal, slang, ragam Twitter) justru
  menyasar dimensi yang tidak menentukan di sini.
  Dan memakai representasi teks TERBAIK pun cuma menggerakkan meta +0.00088,
  sementara SE di 4000 user +-0.0028.
  Jadi tebakan saya: masuk pita yang sama, bobot meta mendekati nol.
  Run Anda yang memutuskan, bukan saya.

KELUARAN (empat, dari fit base yang sama)
  submission_v39_dua.csv    META v26 + listnet + embedding
  submission_v39_lnet.csv   META v26 + listnet          (= v35_lnet, LB 0.66113)
  submission_v39_emb.csv    META v26 + embedding
  submission_v39_v26.csv    META v26 murni              (pembanding)

WAJIB SEBELUM KIRIM: bandingkan urutan top-5 file baru thd file lama.
Kalau 100% sama, NDCG@5-nya PERSIS sama dan submit-nya sia-sia. Blok di
akhir skrip sudah melakukannya otomatis.

Runtime ~3 jam CPU + ~5 menit encode.
"""

# Kalau sentence-transformers belum terpasang DAN internet menyala,
# hapus tanda komentar baris berikut (jalankan sekali):
...
# ------------------------------------------------------------- EMBEDDING
# [v39] Encoder IndoBERTweet, dipanggil lewat sentence-transformers.
# Urutan percobaan:
#   1. model lokal yang dilampirkan di /kaggle/input (tidak butuh internet)
#   2. unduh dari hub (butuh Settings -> Internet ON)
#   3. transformers biasa + mean-pooling (kalau sentence-transformers absen)
#   4. lewati seluruh jalur embedding (run tetap sah)
EMB_NAME  = "indolem/indobertweet-base-uncased"
EMB_DIRS  = [
    "/kaggle/input/indobertweet-base-uncased",
    "/kaggle/input/indolem-indobertweet-base-uncased",
    "/kaggle/input/indobertweet",
    "/kaggle/input/indobert-base-p1",
]
EMB_BATCH  = 64
EMB_MAXLEN = 256

def _cari_model():
    """Cari folder model yang dilampirkan di Kaggle (kalau ada)."""
    for base in EMB_DIRS:
        p = Path(base)
        if not p.exists(): continue
        for q in [p]+sorted(x for x in p.rglob("*") if x.is_dir()):
            if (q/"config.json").exists() and any(
               (q/f).exists() for f in ("pytorch_model.bin","model.safetensors")):
                return str(q)
    return None

def _encode_semua(teks):
    """Kembalikan matriks embedding (baris ternormalkan) atau None."""
    lokal = _cari_model()
    # --- jalur 1/2: sentence-transformers (persis dokumentasi HF) ---
    try:
        from sentence_transformers import SentenceTransformer
        for sumber in [x for x in (lokal, EMB_NAME) if x]:
            try:
                print(f"EMBEDDING: SentenceTransformer('{sumber}')",flush=True)
                model = SentenceTransformer(sumber)
                E = model.encode(teks, batch_size=EMB_BATCH,
                ...
                print(f"           gagal di '{sumber}' ({type(e).__name__}: {e})")
    except ImportError:
        print("EMBEDDING: sentence-transformers tidak ada, coba transformers biasa.")
    # --- jalur 3: transformers + mean-pooling manual ---
    try:
        import torch
        from transformers import AutoTokenizer, AutoModel
        sumber = lokal or EMB_NAME
        print(f"EMBEDDING: transformers mean-pooling di '{sumber}'",flush=True)
        dev = "cuda" if torch.cuda.is_available() else "cpu"
        tok = AutoTokenizer.from_pretrained(sumber)
        mdl = AutoModel.from_pretrained(sumber).to(dev).eval()
        out = []
        with torch.no_grad():
            for i in range(0, len(teks), EMB_BATCH):
                b = tok(teks[i:i+EMB_BATCH], padding=True, truncation=True,
                        max_length=EMB_MAXLEN, return_tensors="pt").to(dev)
                h = mdl(**b).last_hidden_state
                m = b["attention_mask"].unsqueeze(-1).float()
                out.append(((h*m).sum(1)/m.sum(1).clamp(min=1e-9)).cpu().numpy())
        del mdl
        if dev == "cuda": torch.cuda.empty_cache()
        return normalize(np.vstack(out).astype(np.float64))
    except Exception as e:
        print(f"EMBEDDING: gagal total ({type(e).__name__}: {e})")
    return None

HAS_EMB=False; X_EMB=None; EMB_SIM=None
# model ini "-uncased": tokenizer-nya memang mengecilkan huruf, tapi kita
# lakukan eksplisit supaya perilakunya tidak bergantung config tokenizer.
_TEKS = [t.lower() for t in (docs + mod_text)]
_E_ALL = _encode_semua(_TEKS)                    # user dulu, lalu 17 modul
if _E_ALL is not None:
    X_EMB   = _E_ALL[:len(docs)]
    _E_MOD  = _E_ALL[len(docs):]
    # cosine similarity user x modul; keduanya sudah dinormalkan -> dot product
    EMB_SIM = X_EMB @ _E_MOD.T
    HAS_EMB = True
    print(f"EMBEDDING: OK. user {X_EMB.shape}, modul {_E_MOD.shape}, "
          f"sim {EMB_SIM.shape}")
else:
    print("EMBEDDING: jalur embedding DILEWATI (lihat peringatan di penutup).")
...
def train_predict(tr_users, pr_users_list, seeds):
    ...
    if HAS_EMB:
        eclf=LogisticRegression(C=4.0,max_iter=1000).fit(X_EMB[rw],dmi)
        ereg=Ridge(alpha=1.0,solver="lsqr").fit(X_EMB[rw],Ytr_w)
        def _pr_emb(m,rows):
            q=m.predict_proba(X_EMB[rows]); Z=np.zeros((q.shape[0],17))
            Z[:,m.classes_.astype(int)]=q; return Z
    ...
    for pr_users in pr_users_list:
        ...
        o=o.merge(w2l(fit_listnet(_tr,_pv,seeds[:LNET_SEEDS],"list"),_pu,"pred_lnet"),
                  on=["user_id","module_id"],how="left")
        if HAS_EMB:
            o=o.merge(w2l(_pr_emb(eclf,rw_p),u_p,"pred_emb_clf"),
                      on=["user_id","module_id"],how="left")
            o=o.merge(w2l(np.clip(ereg.predict(X_EMB[rw_p]),0,1),u_p,"pred_emb"),
                      on=["user_id","module_id"],how="left")
            # sinyal TANPA training: kemiripan makna user <-> katalog modul,
            # dinormalkan per user supaya skalanya sebanding dgn sinyal lain
            _S=EMB_SIM[rw_p]
            _S=(_S-_S.min(1,keepdims=True))/(_S.max(1,keepdims=True)
                                             -_S.min(1,keepdims=True)+1e-9)
            o=o.merge(w2l(_S,u_p,"pred_emb_sim"),
                      on=["user_id","module_id"],how="left")
...
META_LN =META+["pred_lnet"]                                          # v35 (LB 0.66113)
_E      =["pred_emb_sim","pred_emb_clf","pred_emb"] if HAS_EMB else []
META_EM =META+_E                                                     # [v39] embedding
META_2  =META+["pred_lnet"]+_E                                       # [v39] keduanya
...
if RUN_SEALED:
    ...
    for c,cols in [("s24",META_v24),("s26",META),("sLN",META_LN),
                   ("sLM",META_EM),("s2",META_2)]:
        rg=Ridge(alpha=1.0,positive=True).fit(oof_s[cols].fillna(0),oof_s["target"])
        p[c]=np.clip(rg.predict(p[cols].fillna(0)),0,1)
    for c,nm in [("s24","META v24"),("s26","META v26"),("sLN","+ listnet"),
                 ("sLM","+ embedding"),("s2","+ KEDUANYA")]:
        print(f"  {nm:24s}: NDCG@5 = {ndcg_of(p,c):.5f}  (linear {ndcg_of(p,c,False):.5f})")
    _b=ndcg_of(p,"s26")
    print("\n  selisih thd META v26 (SE satu pengukuran di 1000 user ~ +-0.0015):")
    _best=("s26","META v26",0.0)
    for c,nm in [("sLN","+ listnet"),("sLM","+ embedding"),("s2","+ keduanya")]:
        _v=ndcg_of(p,c)-_b; print(f"    {nm:16s}: {_v:+.5f}")
        if _v>_best[2]: _best=(c,nm,_v)
    print(f"\n  PEMENANG di holdout tersegel: {_best[1]} ({_best[2]:+.5f})")
    print(f"  -> kirim submission_v39_{ {'s26':'v26','sLN':'lnet','sLM':'emb','s2':'dua'}[_best[0]] }.csv")
    ...
    print(f"  kemiripan top-5 thd v24: v26 "
          ...
          f"+keduanya {np.mean([set(x)==set(y) for x,y in zip(_t('s24'),_t('s2'))]):.3f}")
    print("="*70)
...
if len(reps)>1:
    ...
    for cols,nm in [(META_v24,"META v24"),(META,"META v26"),(META_LN,"+ listnet"),
                    (META_EM,"+ embedding"),(META_2,"+ keduanya")]:
...
for c in META_2: pALL[c]=np.mean([banks[k][c].to_numpy() for k in banks],0)
...
print("\nMenulis submission:")
s2=tulis(META_2,"v39_dua"); sL=tulis(META_LN,"v39_lnet")
sM=tulis(META_EM,"v39_emb"); sV=tulis(META,"v39_v26")
tp=lambda s: np.argsort(-s[MODULE_COLS].to_numpy(),1)[:,:5]
ident=lambda x,y: np.mean([set(p)==set(q) for p,q in zip(x,y)])
print(f"\n  v39_dua vs v39_v26 : top-5 set identik {ident(tp(s2),tp(sV)):.3f}")
print(f"  v39_dua vs v39_lnet: top-5 set identik {ident(tp(s2),tp(sL)):.3f}")

# ---- WAJIB: cek apakah file baru benar-benar BARU (urutan top-5) ----
import glob, os
def _ord5(d): return np.argsort(-d[MODULE_COLS].to_numpy(),1)[:,:5]
print("\nCek duplikat thd submission lama (NDCG@5 hanya bergantung URUTAN):")
_lama=sorted(glob.glob("submission_*.csv"))+sorted(glob.glob("/kaggle/input/**/submission_*.csv",recursive=True))
_baru={"v39_dua":s2,"v39_lnet":sL,"v39_emb":sM,"v39_v26":sV}
if not _lama:
    print("  (tidak ada file submission lama di lingkungan ini -> lewati)")
for nm,dfb in _baru.items():
    ob=_ord5(dfb); kembar=[]
    for p in _lama:
        try:
            do=pd.read_csv(p).sort_values("user_id").reset_index(drop=True)
            if len(do)!=len(dfb) or (do.user_id.values!=dfb.user_id.values).any(): continue
            if np.mean([(x==y).all() for x,y in zip(ob,_ord5(do))])==1.0:
                kembar.append(os.path.basename(p))
        except Exception: pass
    print(f"  {nm:9s}: "+("DUPLIKAT dari "+", ".join(kembar)+" -> JANGAN KIRIM"
                          if kembar else "file baru"))
...
CARA MEMBACA HASIL RUN INI
  ...
  model pretrained-nya TIDAK terpasang. Dalam kondisi itu:
      v39_emb == v39_v26   dan   v39_dua == v39_lnet == v35_lnet
