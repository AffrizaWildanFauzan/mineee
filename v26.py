"""
MineToday — Module Relevance Prediction (v26)
=============================================
Riwayat LB: v11 0.65725 | v21 0.65888 | v22 0.65670 | v23 0.65827 |
            v24 0.66080 | v25 0.65848

v26 ditulis SETELAH mengukur langsung di dataset (bukan menebak dari LB).
Delapan ide diuji dgn 5-fold berpasangan + "pseudo-test" (1000 user train
ditahan, rasio persis seperti train:test asli). Hasilnya:

  ide yg diuji                              delta NDCG@5      putusan
  ----------------------------------------  ---------------   -------
  denoise target (buang jitter +-0.05)      +0.0001           nol
  fitur posisi penyebutan modul             -0.0005..-0.0015  RUGI
  prediksi test = rata-rata model fold      -0.0010 (2/2)     RUGI
  ...
  pred_text (andalan v24)                   +0.0001           noise
  pred_text_clf                             +0.0004 (7/10)    dipakai

  sebaran NDCG antar-SEED model yg sama     sd = 0.00085

Kesimpulan yang menentukan: SEMUA selisih antar-versi (v21/v24/v25) besarnya
SAMA dengan selisih antara dua seed acak dari model yang sama. Ditambah public
LB cuma ~31% test (~310 user, SE +-0.014), gerakan LB 0.658<->0.661 selama ini
adalah NOISE, bukan kemajuan atau kemunduran.

Karena itu v26 tidak mengejar sinyal baru (sudah mentok), melainkan:
  1. Base = v24 dibekukan (sweep hyperparameter mengonfirmasi setelan v24
     sudah mendekati optimal; nl15/nl63/lr.015/nl7 semuanya lebih buruk).
  2. + pred_text_clf  -- satu-satunya tambahan yang tandanya konsisten positif
     (+0.0004, menang 7 dari 10 split pseudo-test).
  3. Ridge saja. Meta-ranker DIBUANG: 12 split memberi +0.0005 +-0.0005,
     tidak sepadan dgn tambahan komponen yang bisa gagal.
  4. Seed bagging diperbanyak (8) di retrain final. Ini satu-satunya tuas yang
     secara teori tidak bisa merugikan: hanya menurunkan varians prediksi.
     Diukur: rata-rata 8 seed +0.0002 dibanding seed acak tunggal.
  5. Repeated CV 4x supaya bobot Ridge lebih stabil.
  6. BLOK PSEUDO-TEST BAWAAN: menahan 1000 user train, melatih ulang seluruh
     pipeline di 3000 sisanya, lalu melaporkan NDCG di 1000 user yang belum
     pernah dilihat. Ini estimasi yang jujur untuk "seberapa bagus di test",
     dan alat yang benar untuk menguji ide berikutnya -- bukan CV, bukan LB.
...
out=OUT_DIR/"submission_v26.csv"; submission.to_csv(out,index=False)
