"""
=======================================================================
 MineToday v37 -- konsolidasi varians-minimum, berhenti mengejar LB publik
=======================================================================
BUKTI DARI 55 SUBMISSION TIM:
  - Rekor tim 0.66118 (v29_a) berumur 5 hari; v34/v35/v36 SEMUANYA <= itu.
  - v29a vs v35_lnet: 86% set top-5 identik, 95% rank-1 identik,
    Spearman skor 0.993. Seminggu terakhir tim mengirim near-copy.
  - Jarak ke rank 5 (0.66134) = +0.00016. SE satu pengukuran NDCG@5 di
    ~310 user LB publik ~ +-0.0025. Jadi selisih rank 2..6 (0.66193..
    0.66118, rentang 0.00075) SELURUHNYA di dalam pita noise. Urutan
    papan publik untuk tim-tim ini praktis undian.
  - Ledger v29: 5 ide menyerang bottleneck retrieval, 5-5nya rugi. Tidak
    ada perubahan MODEL terukur yang mendekati +0.0025 yg dibutuhkan
    untuk *yakin* lewat 0.66142.
  - rank 1 = 0.966 (vs 0.66 semua orang) -> hampir pasti overfit ke set
    publik / kebocoran. Yang menentukan finalis adalah PRIVATE LB (690
    user). Di sana strategi benar = ekspektasi tertinggi + varians
    terendah, BUKAN jumlah tiket undian LB publik.

APA YANG v37 UBAH (hanya SATU hal, sisanya = v36 apa adanya):
  Submission akhir tidak lagi "pilih satu kepala meta". Keputusan
  "META v24 vs v26 vs +listwise" itu -- kata docstring v29 sendiri --
  lemparan koin yang menyuntik +0.0011..0.0016 CV PALSU. v37 tidak
  memilih: submission = rata-rata TIGA kepala meta yang dibangun dari
  base model yang sama:
      H1  META_v24                    (7 sinyal, set "pra-bakar" jujur)
      H2  META_v24 + pred_lnet_lam    (+ SATU sinyal baru yg positif di
                                       KEDUA protokol validasi v36:
                                       +0.00058 skeptis / +0.0013 optimis)
      H3  META_2  (10 sinyal)         (CV terbaik; sebagian inflasi seleksi)
  Rentang H1->H3 = konservatif->agresif. Rata-rata membuang sumber
  varians terbesar (pilihan meta-set) tanpa mengorbankan ekspektasi.
  EV private >= kepala tunggal mana pun; varians jelas lebih rendah.

  [OPSIONAL, default nyala kalau file ada] ANCHOR: kalau
  submission_v29_a_metaV24.csv ditambahkan sbg input dataset, v37
  mencampur peringkat-per-user-nya dgn bobot ANCHOR_W=0.30 -- pengunci
  matematis supaya v37 tidak bisa melenceng jauh dari titik 0.66118
  yang sudah TERBUKTI. Kalau file tak ada, blok ini dilewati.

KELUARAN:
  submission_v37_main.csv     <- KIRIM INI (blend 3 kepala + anchor)
  submission_v37_v24.csv      komponen H1  (cek vs holdout tersegel)
  submission_v37_v24lam.csv   komponen H2
  submission_v37_dua.csv      komponen H3
DUA SLOT FINAL:
  slot 1 -> submission_v37_main.csv
  slot 2 -> submission_v29_a_metaV24.csv  (rekor lama, kepala meta beda;
            Kaggle ambil yg terbaik dari dua yg dipilih di private LB)

Runtime ~3-3.5 jam di kernel CPU Kaggle (dua model listwise per fold).
Tidak butuh internet. Seed reproducible (SEED=42) dicatat di semua model.
"""
...
# --- v37: blend kepala meta + anchor -----------------------------------
ANCHOR_FILES = ["submission_v29_a_metaV24.csv", "submission_v36_lnet.csv"]
ANCHOR_W     = 0.30          # 0.0 = matikan anchor; 0.30 = pengunci lembut
...
# ---------------------------------------------------------------- LISTWISE
# Scorer bersama menilai tiap pasangan (user,modul) dgn 43 fitur long-format
# + one-hot modul, lalu 17 skor satu user di-softmax. Dua objective:
#   mode="list"   ListNet   -- cross-entropy softmax gain vs softmax skor
#   mode="lambda" LambdaRank-- loss pairwise berbobot |dNDCG@5|, langsung
#                 menyasar metrik lomba. Korelasi dgn pred_reg cuma 0.819
#                 -> paling tidak berkorelasi -> paling berguna utk ensemble.
# Numpy murni, tanpa dependensi.
...
META_LN =META+["pred_lnet"]                                          # v35
META_LM =META+["pred_lnet_lam"]                                      # v36 lambda
META_2  =META+["pred_lnet","pred_lnet_lam"]                          # v36 keduanya
# --- v37: TIGA kepala yang dirata-rata jadi submission akhir -----------
H1=META_v24                       # set "pra-bakar" jujur (7)
H2=META_v24+["pred_lnet_lam"]     # + 1 sinyal baru, positif di 2 protokol (8)
H3=META_2                         # semua sinyal, CV terbaik (10)
BLEND_HEADS=[("v37_v24",H1),("v37_v24lam",H2),("v37_dua",H3)]
...
def ridge_blend(train_meta, pred_df, head_sets):
    """Fit Ridge tiap kepala di train_meta, prediksi pred_df, rata-ratakan skornya."""
...
if RUN_SEALED:
    ...
    print("oleh ~30 keputusan seleksi yang diambil di data train.")
    ...
    print(f"  kemiripan top-5 BLEND thd META v24: "
          f"{np.mean([set(x)==set(y) for x,y in zip(_t('s24'),_t('blend'))]):.3f}")
    print("  -> kalau v37 BLEND >= 0 vs META v24 di sini: kirim submission_v37_main.csv")
    print("     kalau < 0 secara jelas (> 1 SE): kirim submission_v29_a_metaV24.csv lama")
    print("="*70)
...
if len(reps)>1:
    ...
    e,l=loo_blend(); print(f"  {'v37 BLEND':16s}: exp={e:.5f}  linear={l:.5f}")
...
pALL["module_id"]=pALL["module_id"].astype(str)          # buang dtype category -> merge/pivot aman
for c in META_2: pALL[c]=np.mean([banks[k][c].to_numpy() for k in banks],0)

# --- tiga kepala meta + blend ---
...
# --- ANCHOR opsional: campur peringkat-per-user dgn submission terbukti ---
def _pct(df,col): return df.groupby("user_id",observed=True)[col].rank(pct=True)
pALL["r_blend"]=_pct(pALL,"blend")
anchor=None; apath=None
_search=[DATA_DIR, DATA_DIR.parent, Path("/kaggle/input"), OUT_DIR, Path(".")]
for fn in ANCHOR_FILES:
    for base in _search:
        try: hits=sorted(Path(base).rglob(fn))
        except Exception: hits=[]
        if hits: anchor,apath=pd.read_csv(hits[0]),hits[0]; break
    if anchor is not None: break
if anchor is not None and ANCHOR_W>0:
    print(f"\nANCHOR dipakai (W={ANCHOR_W}): {apath}")
    al=anchor.melt(id_vars="user_id",var_name="module_id",value_name="anchor")
    pALL=pALL.merge(al,on=key,how="left")
    if pALL["anchor"].isna().any():
        print("  ! anchor tidak lengkap utk semua user/modul -> anchor DIBATALKAN")
        pALL["final"]=pALL["r_blend"]
    else:
        pALL["r_anchor"]=_pct(pALL,"anchor")
        pALL["final"]=(1-ANCHOR_W)*pALL["r_blend"]+ANCHOR_W*pALL["r_anchor"]
else:
    print(f"\nANCHOR dilewati (file {ANCHOR_FILES} tidak ditemukan atau W=0).")
    print("  Tambahkan submission_v29_a_metaV24.csv sbg input dataset utk mengaktifkan.")
    pALL["final"]=pALL["r_blend"]

def tulis(colname,nama):
    d=pALL[key].copy(); d["pred"]=pALL[colname].to_numpy()
...
print("\nMenulis submission:")
s_main=tulis("final","v37_main")
for hcol,(nm,_) in zip(_hc,BLEND_HEADS): tulis(hcol,nm)
tp=lambda s: np.argsort(-s[MODULE_COLS].to_numpy(),1)[:,:5]
ident=lambda x,y: np.mean([set(a)==set(b) for a,b in zip(x,y)])
_v24cmp=pALL.pivot(index="user_id",columns="module_id",values="_h_v37_v24").reset_index()
_v24cmp=test_ids[["user_id"]].merge(_v24cmp,on="user_id",how="left")[["user_id"]+MODULE_COLS]
print(f"\n  v37_main vs H1(v24)  : top-5 set identik {ident(tp(s_main),tp(_v24cmp)):.3f}")

print("""
==================================================================
CARA MEMBACA HASIL RUN INI

  1. Blok HOLDOUT TERSEGEL di awal: 1000 user yg tak pernah dipakai
     untuk keputusan apa pun. Baris "v37 BLEND" vs "META v24":
       >= 0            -> kirim submission_v37_main.csv
       < -0.0015 jelas -> kirim submission_v29_a_metaV24.csv (rekor lama)
       di antaranya    -> keduanya setara; kirim v37_main (varians lebih
                          rendah krn 3 kepala dirata-rata + anchor)

  2. DUA SLOT FINAL (Kaggle ambil yg terbaik di PRIVATE LB):
       slot 1 -> submission_v37_main.csv
       slot 2 -> submission_v29_a_metaV24.csv

  3. JANGAN kirim ulang tiap CV naik < 0.001. Jarak ke rank 5 di papan
     PUBLIK (+0.00016) < 1/10 SE -- itu undian. Yg menentukan finalis
     adalah PRIVATE LB 690 user; di sana submission varians-terendah
     (v37_main) punya ekspektasi peringkat terbaik.
==================================================================""")
