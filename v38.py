"""
=======================================================================
 MineToday v38 -- sentence-transformers (dokumentasi resmi HuggingFace)
=======================================================================
Encoder teks diganti/didampingi model kalimat pretrained multibahasa,
dipanggil persis seperti dokumentasi HuggingFace:
...
TIGA SINYAL BARU (semuanya dari embedding yang sama)
  pred_emb_sim  cosine(embedding user, embedding katalog modul).
                TANPA dilatih sama sekali -- murni pengetahuan bahasa
                dari model pretrained. Ini padanan semantik dari fitur
                tfidfword_/tfidfchar_ yang sekarang cuma cocok-kata.
                Di sinilah embedding punya peluang terbesar: user yang
                memparafrase ("pengen bisa bikin grafik yang bagus")
                tidak kena regex keyword saya, tapi kena di ruang makna.
  pred_emb_clf  LogisticRegression di embedding -> modul dominan
                (padanan langsung pred_text_clf, tapi semantik)
  pred_emb      Ridge di embedding -> 17 nilai relevansi

CARA MENJALANKAN DI KAGGLE -- pilih SATU:
  (A) PALING GAMPANG: Settings -> Internet -> ON. Model diunduh otomatis
      (~470 MB, sekali saja). Kalau sentence-transformers belum ada,
      hapus tanda komentar di baris !pip install di bawah.
  (B) TANPA INTERNET: Add Input -> Models -> cari
      "paraphrase-multilingual-MiniLM-L12-v2", lalu run seperti biasa.
      Kode mencari model lokal DULU sebelum mencoba unduh.
  Kalau dua-duanya gagal, jalur embedding DILEWATI dan run tetap
  menghasilkan submission yang sah (tapi lihat peringatan di penutup).
  GPU opsional -- CPU sekitar 2-4 menit untuk 5000 dokumen.

EKSPEKTASI SAYA, JUJUR
  Saya sudah mengukur plafonnya (e65-e67) dan hasilnya bilang ruangnya
  tipis: saat user menyebut SATU modul model sekarang sudah benar 96.1%,
  dan user dgn chat IDENTIK punya modul rank-1 BERBEDA di 62-75% kasus
  -- tembok itu bukan soal pemahaman bahasa, jadi encoder secanggih apa
  pun tidak bisa menembusnya.
  Satu celah yang tersisa: 1082 user yang modul aslinya TIDAK terdeteksi
  keyword saya (akurasi cuma 13.9%). pred_emb_sim dirancang khusus untuk
  celah itu. Apakah cukup untuk menggeser NDCG@5? Tidak tahu -- blok
  holdout tersegel di awal output yang memutuskan, bukan saya.

KELUARAN (empat, dari fit base yang sama)
  submission_v38_dua.csv    META v26 + listnet + embedding
  submission_v38_lnet.csv   META v26 + listnet            (= v35, LB 0.66113)
  submission_v38_emb.csv    META v26 + embedding
  submission_v38_v26.csv    META v26 murni                (pembanding)

Runtime ~3 jam (CPU) + ~3 menit encode.
"""
...
# ------------------------------------------------------------- EMBEDDING
# [v38] Encoder kalimat pretrained, dipanggil lewat sentence-transformers
# sesuai dokumentasi resmi HuggingFace. Urutan percobaan:
...
EMB_DIRS  = [
    "/kaggle/input/paraphrase-multilingual-minilm-l12-v2",
    "/kaggle/input/sentence-transformers-paraphrase-multilingual-minilm-l12-v2",
    "/kaggle/input/minilm-multilingual",
...
_E_ALL = _encode_semua(docs + mod_text)          # user dulu, lalu 17 modul
...
META_EM =META+_E                                                     # [v38] embedding
META_2  =META+["pred_lnet"]+_E                                       # [v38] keduanya
...
if RUN_SEALED:
    ...
    print(f"  -> kirim submission_v38_{ {'s26':'v26','sLN':'lnet','sLM':'emb','s2':'dua'}[_best[0]] }.csv")
...
print("\nMenulis submission:")
s2=tulis(META_2,"v38_dua"); sL=tulis(META_LN,"v38_lnet")
sM=tulis(META_EM,"v38_emb"); sV=tulis(META,"v38_v26")
tp=lambda s: np.argsort(-s[MODULE_COLS].to_numpy(),1)[:,:5]
ident=lambda x,y: np.mean([set(p)==set(q) for p,q in zip(x,y)])
print(f"\n  v38_dua vs v38_v26 : top-5 set identik {ident(tp(s2),tp(sV)):.3f}")
print(f"  v38_dua vs v38_lnet: top-5 set identik {ident(tp(s2),tp(sL)):.3f}")
...
CARA MEMBACA HASIL RUN INI
  ...
  model pretrained-nya TIDAK terpasang. Dalam kondisi itu:
      v38_emb == v38_v26   dan   v38_dua == v38_lnet == v35_lnet
