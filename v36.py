"""
=======================================================================
 MineToday v36 -- dua objective listwise, keputusan diserahkan ke run
=======================================================================
v35 (listwise ListNet) mencetak 0.66113 di papan publik -- tertinggi ke-2
dari 14 submission, dan +0.00074 di atas META v26 murni. v36 menambah
objective listwise KEDUA dan membiarkan holdout tersegel yang memilih.

DUA OBJECTIVE:
  pred_lnet      ListNet  -- cross-entropy softmax gain vs softmax skor
  pred_lnet_lam  LambdaRank -- loss pairwise berbobot |dNDCG@5|, jadi
                 langsung menyasar metrik lomba, bukan seluruh daftar

HASIL PENGUKURAN (100 fold berpasangan di 3000 user DEV):
  META v26 (patokan)     0.66662
  + listnet              0.66799   +0.00137   7.1 sigma
  + lambdarank           0.66792   +0.00131   9.3 sigma
  + KEDUANYA             0.66825   +0.00164   8.3 sigma   <- terbaik
  lambdarank sendirian 0.65520 (terbaik di keluarganya), korelasi dgn
  pred_reg hanya 0.819 -- paling tidak berkorelasi, itu sebabnya berguna.

DUA VARIAN LAIN SUDAH DIUJI DAN DIBUANG:
  + fitur teks SVD (hybrid)  +0.00022  1.5 sigma   lemah
  + target one-hot rank-1    +0.00078  5.6 sigma   kalah dari dua di atas

KETIDAKPASTIAN YANG BELUM SAYA SELESAIKAN. Di holdout tersegel saya punya
dua protokol yang HASILNYA BERBEDA untuk listnet:
  blok tersegel di dalam pipeline (v35): +0.00219, +0.00256, +0.00297
  penggabungan OOF lintas-run (e56/e60): -0.00049
Saya sudah memeriksa dugaan paling jelas -- ketidaksejajaran fold -- dan
ternyata urutan barisnya identik, jadi bukan itu penyebabnya. Sumbernya
belum saya temukan. Yang bisa saya katakan: lambdarank positif di KEDUA
protokol (+0.00058 di protokol yang skeptis), listnet hanya di satu.
Di papan publik nyata, v35_lnet 0.66113 vs META v26 0.66038 = +0.00074.

KELUARAN (empat, dari fit base yang sama):
  submission_v36_dua.csv    META v26 + listnet + lambdarank
  submission_v36_lnet.csv   META v26 + listnet          (= v35)
  submission_v36_lam.csv    META v26 + lambdarank
  submission_v36_v26.csv    META v26 murni              (pembanding)

CARA PAKAI: jalankan, lalu BACA BLOK HOLDOUT TERSEGEL di awal output. Ia
mencetak keempat varian dan menyebut pemenangnya. Kirim yang itu.
...
if RUN_SEALED:
    ...
    for c,nm in [("sLN","+ listnet"),("sLM","+ lambdarank"),("s2","+ keduanya")]:
        _v=ndcg_of(p,c)-_b; print(f"    {nm:16s}: {_v:+.5f}")
        if _v>_best[2]: _best=(c,nm,_v)
    print(f"\n  PEMENANG di holdout tersegel: {_best[1]} ({_best[2]:+.5f})")
    print(f"  -> kirim submission_v36_{ {'s26':'v26','sLN':'lnet','sLM':'lam','s2':'dua'}[_best[0]] }.csv")
...
print("\nMenulis submission:")
s2=tulis(META_2,"v36_dua"); sL=tulis(META_LN,"v36_lnet")
sM=tulis(META_LM,"v36_lam"); sV=tulis(META,"v36_v26")
tp=lambda s: np.argsort(-s[MODULE_COLS].to_numpy(),1)[:,:5]
ident=lambda x,y: np.mean([set(p)==set(q) for p,q in zip(x,y)])
print(f"\n  v36_dua vs v36_v26 : top-5 set identik {ident(tp(s2),tp(sV)):.3f}")
print(f"  v36_dua vs v36_lnet: top-5 set identik {ident(tp(s2),tp(sL)):.3f}")
