...
# ---------------------------------------------------------------------
# E35: HIPOTESIS ATENUASI.
# Bobot meta-Ridge dilatih di OOF yang base model-nya 1 SEED (bising),
# lalu DIPAKAI pada prediksi test yang base model-nya 20 SEED (mulus).
# pred_knn / pred_text / pred_text_clf TIDAK punya varians seed sama sekali,
# sedangkan lima model pohon punya. Jadi di OOF model pohon terlihat lebih
# bising daripada saat dipakai -> Ridge sistematis MENGECILKAN bobot pohon
# dan MEMBESARKAN bobot teks (errors-in-variables attenuation).
# Kalau benar, bobot pred_text_clf akan TURUN saat OOF dihitung multi-seed,
# dan itu menjelaskan kenapa CV suka v26 tapi LB (test) suka v24.
# Diuji: OOF dev 1 seed vs 4 seed -> Ridge -> dinilai di 1000 user tersegel
# yang prediksinya memang 4 seed.
# ---------------------------------------------------------------------
...
full_s.to_pickle("exp/cache/sealed_full.pkl")

import time; t0=time.time()
for nseed in (1,4):
    oof_s=run_oof(devu,[SEED],SEED_BANK_A[:nseed])[0]
    oof_s.to_pickle(f"exp/cache/sealed_oof_{nseed}seed.pkl")
    print(f"\n=== OOF dev dgn {nseed} seed per fold ({time.time()-t0:.0f}s) ===",flush=True)
    for cols,nm in [(META_v24,"META v24"),(META,"META v26")]:
        ...
        print(f"  {nm}: tersegel={ndcg_of(p,'s'):.5f}  koef={dict(zip(cols,np.round(rg.coef_,3)))}")
