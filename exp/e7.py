"""E7: lanjutan E3 -- hanya FULLFIT, lebih banyak kandidat meta & bobot blend."""
...
for split in range(NSPLIT):
    t0=time.time()
    rng=np.random.RandomState(200+split); perm=rng.permutation(len(allu))
    ...
    for cols,cn in [(META_A,"A_v21"),(META,"B_v24"),(META_C,"C_text_clf")]:
        ...
        rn,kn2=nrm(p,"_rg"),nrm(p,"_rk")
        for w in [1.0,0.75,0.5,0.25,0.0]:
            p["_b"]=rn if w>=1 else (kn2 if w<=0 else w*rn+(1-w)*kn2)
            rows.append(dict(split=split,meta=cn,w_ridge=w,ndcg=ndcg_of(p,"_b")))
    pd.DataFrame(rows).to_csv("/home/user/mineee/exp/e7_results.csv",index=False)
    print(f"split {split} SELESAI ({time.time()-t0:.0f}s)",flush=True)

R=pd.DataFrame(rows)
print("\n=== NDCG@5 PSEUDO-TEST (FULLFIT), rata-rata split ===")
print(R.pivot_table(index="meta",columns="w_ridge",values="ndcg").round(5))
