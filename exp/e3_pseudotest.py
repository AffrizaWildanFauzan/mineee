"""E3: pseudo-test (tahan 1000 user train, rasio persis 4000:1000).
Bandingkan cara memprediksi 'test':
  FULLFIT : base model dilatih ulang di SEMUA dev-train  (cara v11-v25)
  FOLDAVG : rata-rata prediksi 5 model fold
dan meta-model: Ridge murni vs Ridge+meta-ranker (yg dipilih v25).
Setiap set model dilatih SEKALI lalu dipakai memprediksi beberapa set sekaligus.
"""
...
META=["pred_reg","pred_rank","pred_reg_xgb","pred_rank_xgb","clf_proba","pred_knn","pred_text"]
META_C=META+["pred_text_clf"]
...
for split in range(NSPLIT):
    t0=time.time()
    rng=np.random.RandomState(100+split); perm=rng.permutation(len(allu))
    ...
    favg=fold_pt[0][KEY+["target"]].copy()
    for c in META_C: favg[c]=np.mean([fp[c].to_numpy() for fp in fold_pt],axis=0)
    for cols,cn in [(META,"B_v24"),(META_C,"C_text_clf")]:
        rg=Ridge(alpha=1.0,positive=True).fit(oof[cols].fillna(0),oof["target"])
        os_=oof.sort_values(["user_id"],kind="stable")
        mr=lgb.LGBMRanker(objective="lambdarank",metric="ndcg",eval_at=[5],n_estimators=300,learning_rate=0.05,
            num_leaves=15,subsample=0.9,colsample_bytree=0.9,random_state=7,verbose=-1,n_jobs=4).fit(
            os_[cols].fillna(0),os_["target"].map(to_grade).astype(int),
            group=os_.groupby("user_id",observed=True).size().values)
        for mode,dfp in [("FULLFIT",full),("FOLDAVG",favg)]:
            p=dfp.copy()
            p["_rg"]=np.clip(rg.predict(p[cols].fillna(0)),0,1); p["_rk"]=mr.predict(p[cols].fillna(0))
            rows.append(dict(split=split,meta=cn,mode=mode,model="ridge",ndcg=ndcg_of(p,"_rg")))
            p["_bl"]=0.5*nrm(p,"_rg")+0.5*nrm(p,"_rk")
            rows.append(dict(split=split,meta=cn,mode=mode,model="ridge+ranker.5",ndcg=ndcg_of(p,"_bl")))
    pd.DataFrame(rows).to_csv("/home/user/mineee/exp/e3_results.csv",index=False)
    print(f"split {split} SELESAI ({time.time()-t0:.0f}s)",flush=True)

R=pd.DataFrame(rows)
print("\n=== NDCG@5 di PSEUDO-TEST (rata-rata split) ===")
print(R.pivot_table(index=["meta","model"],columns="mode",values="ndcg").round(5))
