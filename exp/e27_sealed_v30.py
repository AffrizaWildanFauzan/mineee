"""
MineToday — Module Relevance Prediction (v29)
=============================================
Riwayat LB: v11 .65725 | v21 .65888 | v22 .65670 | v23 .65827 | v24 .66080
            v25 .65848 | v26 .65950 | v27 .65788 | v28 .65430   (rank-1 = .66142)

v29 BUKAN model baru. Ini konfigurasi v24 -- satu-satunya yang META set-nya
TIDAK saya pilih dari data yang sudah saya bakar -- dijalankan dgn varians
serendah mungkin, plus evaluasi di 1000 user TERSEGEL yang belum pernah
disentuh sama sekali.

KENAPA BUKAN MODEL BARU. Yang sudah diukur di sesi ini:
  ide                                        delta NDCG@5      putusan
  -----------------------------------------  ----------------  -------
  denoise target                             +0.0001           nol
  fitur posisi penyebutan modul              -0.0005..-0.0015  RUGI
  prediksi test = rata-rata model fold       -0.0010           RUGI
  ...
  meta-ranker diblend dgn Ridge              +0.0005 +-0.0005  noise
  prasyarat berbasis MIN                     +0.0006 +-0.0006  noise
  pred_text_clf                              +0.0004 (7/10)    noise
  GABUNGAN semua yang positif (v27)          +0.0005 +-0.0007  noise

  sebaran NDCG antar-SEED model yang sama    sd = 0.00085
  adversarial AUC train-vs-test              0.4964 (tanpa shift)
  selisih model TERBESAR yang terukur        0.0008
  sd 8 skor LB tim                           0.00137

Lima ide menyerang bottleneck retrieval, lima-limanya rugi. Tidak ada
perubahan model yang terukur mendekati +0.0025 yang dibutuhkan untuk
menembus 0.66142.

KENAPA v24 YANG DIPAKAI, BUKAN v26/v27 yang CV-nya lebih tinggi:
  Saya mengambil ~30 keputusan seleksi di 4000 user yang sama. Tiap fitur
  yang saya pertahankan menang +0.0005 padahal SE-nya juga ~0.0005 -- itu
  lemparan koin. 8-16 keputusan semacam itu menghasilkan kenaikan CV PALSU
  +0.0011..+0.0016; kenaikan CV v24->v27 memang +0.0012. Jadi CV v26/v27
  yang lebih tinggi adalah artefak seleksi saya, dan LB-nya (.65950/.65788)
  konsisten dgn itu. META set v24 dipilih sebelum data terbakar.

YANG BERUBAH DARI v24 (murni penurun varians, tidak mengubah model):
  [+] seed bagging 5 -> 12 pada retrain final
  [+] repeated CV 3 -> 5 (bobot Ridge lebih stabil)
  [+] BLOK TERSEGEL: 1000 user dipisah dgn seed tetap 20260901 dan tidak
      pernah dipakai untuk keputusan apa pun. Dilaporkan sekali di akhir
      sebagai satu-satunya angka yang tidak tercemar seleksi.

EKSPEKTASI JUJUR: rekor tim 0.66080 kemungkinan besar sudah mengandung
~+0.0015 keberuntungan sampling (rata-rata 8 submission = 0.65855).
v29 tidak dirancang untuk mengalahkan 0.66142 -- saya tidak punya cara yang
terukur untuk itu. v29 dirancang untuk punya EKSPEKTASI tertinggi dan
varians terendah, yang berarti peluang terbaik di private LB (690 user).

Cara pakai: 1 cell di Kaggle kernel. Tidak butuh internet. ~40-60 menit.
"""
...
SEEDS        = [42,202,777,2026,31337,7,123,999,1234,5150,8888,31415]  # 12 seed
...
_Yw=train_wide.set_index("user_id")[MODULE_COLS]
_ORD=np.argsort(-_Yw.to_numpy(),axis=1)
RANK2=pd.Series([MODULE_COLS[i] for i in _ORD[:,1]],index=_Yw.index)
RANK3=pd.Series([MODULE_COLS[i] for i in _ORD[:,2]],index=_Yw.index)
...
def train_predict(tr_users, pr_users_list, seeds):
    ...
    # [v30] tiga objective teks tambahan -- korelasi rendah dgn model pohon
    tbin=Ridge(alpha=TEXT_ALPHA,solver="lsqr").fit(X_TEXT[rw],(Ytr_w>0).astype(float))
    d2=np.array([M2I[x] for x in RANK2.reindex(u_tr).values])
    d3=np.array([M2I[x] for x in RANK3.reindex(u_tr).values])
    tclf2=LogisticRegression(C=4.0,max_iter=400).fit(X_TEXT[rw],d2)
    tclf3=LogisticRegression(C=4.0,max_iter=400).fit(X_TEXT[rw],d3)
    ...
    for pr_users in pr_users_list:
        ...
        o=o.merge(w2l(np.clip(tbin.predict(X_TEXT[rw_p]),0,1),u_p,"pred_text_bin"),on=["user_id","module_id"],how="left")
        o=o.merge(w2l(_pr(tclf2),u_p,"pred_text_clf2"),on=["user_id","module_id"],how="left")
        o=o.merge(w2l(_pr(tclf3),u_p,"pred_text_clf3"),on=["user_id","module_id"],how="left")
...
META_v30=META+["pred_text_bin","pred_text_clf2","pred_text_clf3"]    # + keluarga teks
...
if RUN_SEALED:
    ...
    full_s=train_predict(devu,[sealed],SEEDS[:4])[0]
    oof_s.to_pickle("exp/cache/sealed_oof.pkl"); full_s.to_pickle("exp/cache/sealed_full.pkl")
    for cols,nm in [(META_v24,"META v24"),(META,"META v26"),(META_v30,"META v30")]:
...
print("\nCV (leave-one-repeat-out, sebanding dgn angka v21-v25):")
for cols,nm in [(META_v24,"META v24"),(META,"META v26"),(META_v30,"META v30")]:
    e,l=loo(cols); print(f"  {nm}: exp={e:.5f}  linear={l:.5f}")

meta_all=pd.concat(reps,ignore_index=True)
# Dua kandidat dikeluarkan dua-duanya; rekomendasi dicetak di akhir.
META_SETS={"v30_a_metaV26":META,"v30_b_metaV30":META_v30}
RIDGES={nm:Ridge(alpha=1.0,positive=True).fit(meta_all[c].fillna(0),meta_all["target"])
        for nm,c in META_SETS.items()}

# ------------------------------------------------------- PREDIKSI TEST
print(f"\nRetrain final di SELURUH train, {len(SEEDS)} seed bagging...")
test_pred=train_predict(np.array(sorted(train_wide.user_id)),
                        [test_ids.user_id.values], SEEDS)[0]
for nm,rg in RIDGES.items():
    cols=META_SETS[nm]
    test_pred["pred"]=np.clip(rg.predict(test_pred[cols].fillna(0)),0,1)
    sub=(test_pred.pivot(index="user_id",columns="module_id",values="pred")
         .reset_index()[["user_id"]+MODULE_COLS])
    sub=test_ids[["user_id"]].merge(sub,on="user_id",how="left")
    assert sub.shape==(len(test_ids),18) and sub[MODULE_COLS].isna().sum().sum()==0
    sub.to_csv(OUT_DIR/f"submission_{nm}.csv",index=False)
    print(f"\n  {nm}: koef {dict(zip(cols,np.round(rg.coef_,3)))}")
    print(f"    -> {OUT_DIR}/submission_{nm}.csv")
print("\nPILIH berdasarkan blok HOLDOUT TERSEGEL di atas:")
print("  kalau META v30 > META v26  -> kirim submission_v30_b_metaV30.csv")
print("  kalau tidak                -> kirim submission_v30_a_metaV26.csv (= v29b, LB 0.66045)")
