"""E11: apakah RATA-RATA beberapa konfigurasi mengalahkan konfigurasi tunggal terbaik?"""
...
for split in range(NSPLIT):
    t0=time.time()
    rng=np.random.RandomState(300+split); perm=rng.permutation(len(allu))
    ...
    preds={}
    for cols,cn in [(META_A,"A_v21"),(META,"B_v24"),(META_C,"C_text_clf")]:
        ...
        preds[cn+"_ridge"]=nrm(p,"_rg"); preds[cn+"_rank"]=nrm(p,"_rk")
        rows.append(dict(split=split,cfg=cn+"_ridge",ndcg=ndcg_of(p.assign(z=preds[cn+"_ridge"]),"z")))
    def blend(keys,name):
        v=np.mean([preds[k] for k in keys],axis=0)
        rows.append(dict(split=split,cfg=name,ndcg=ndcg_of(full.assign(z=v),"z")))
    blend(["A_v21_ridge","B_v24_ridge","C_text_clf_ridge"],"BLEND_3ridge")
    blend(["B_v24_ridge","C_text_clf_ridge"],"BLEND_BC_ridge")
    blend(["A_v21_ridge","B_v24_ridge","C_text_clf_ridge","A_v21_rank","B_v24_rank","C_text_clf_rank"],"BLEND_6all")
    blend(["B_v24_ridge","B_v24_rank"],"BLEND_B_ridge_rank")
    pd.DataFrame(rows).to_csv("/home/user/mineee/exp/e11_results.csv",index=False)
    print(f"split {split} SELESAI ({time.time()-t0:.0f}s)",flush=True)

R=pd.DataFrame(rows)
print("\n=== E11 rata-rata split ===")
print(R.groupby("cfg")["ndcg"].mean().sort_values(ascending=False).round(5))
