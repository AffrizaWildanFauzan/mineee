...
# ---------------------------------------------------------------------
# E39: SEBERAPA BESAR UNDIAN SEED PADA SATU MODEL YANG SAMA?
# v29_a dan v32_publik_v24 memakai META SET YANG SAMA (v24). Bedanya cuma
# bag seed: 12 vs 24. Skornya 0.66118 vs 0.65998 (-0.00120). Di sini
# diukur langsung: 24 seed dilatih di 3000 user dev, diprediksikan ke 1000
# user tersegel, lalu skor tiap bag seed dibandingkan -- dan disubsampel ke
# 310 user (ukuran LB publik) untuk melihat sebaran yang sebenarnya.
# ---------------------------------------------------------------------
...
GRP=[SEEDS[i*4:(i+1)*4] for i in range(6)]      # 6 grup x 4 seed = 24 seed
t0=time.time(); parts=[]
for gi,g in enumerate(GRP):
    parts.append(train_predict(devu,[sealed],g)[0])
    print(f"  grup {gi+1}/6 selesai ({time.time()-t0:.0f}s)",flush=True)
import pickle; pickle.dump(parts,open("exp/cache/seedbag_parts.pkl","wb"))
print("\nprediksi per-grup disimpan ke exp/cache/seedbag_parts.pkl")
