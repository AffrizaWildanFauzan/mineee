"""
=======================================================================
 MineToday v35 -- pendekatan baru: model LISTWISE di dalam stack
=======================================================================
PERINGATAN DI DEPAN: ide ini TIDAK LOLOS konfirmasi. Angkanya ada di bawah.
Jalankan hanya kalau tim memang ingin mengambil peluang, bukan karena saya
merekomendasikannya.
...
itupun lewat pohon pairwise. v35 menambah satu model dgn struktur berbeda:
  - scorer bersama (MLP 1 hidden layer) menilai tiap pasangan (user,modul)
    dgn 43 fitur long-format + one-hot modul
  ...
  - jadi ranking diperlakukan sbg SATU OBJEK per user, bukan 17 prediksi
    terpisah. Ditulis numpy murni -- tidak butuh torch.

HASIL PENGUKURAN, apa adanya:
  standalone di DEV                : 0.65405  (pred_reg 0.66316)
  korelasi dgn pred_reg            : 0.912
  kontribusi ke stack di DEV       : +0.00127 +- 0.00018, menang 75/100
                                     = 6.9 SIGMA, efek terbesar sepanjang
                                     pengembangan; Ridge memberinya bobot
                                     0.608, terbesar dari semua sinyal
  KONFIRMASI di 1000 user TERSEGEL : -0.00049 +- 0.00148 = -0.3 sigma
                                     TIDAK LOLOS
  gabungan dev + tersegel          : +0.00083 +- 0.00074 (1.1 sigma)

Ini pola yang sama persis dgn ide z-score dulu: 4.8 sigma di dev, lalu nol
di holdout tersegel. Angka dev tinggi sebagian karena arsitektur dan
hyperparameter-nya SAYA PILIH di dev; holdout tersegel tidak tercemar itu.

Sudut keberagaman juga sudah dicek dan buntu. Menambah listwise membuat
prediksi jauh lebih berbeda dari META v24 (kemiripan top-5 turun 0.913 ->
0.769, bagus untuk E[max] dua slot final), tapi reratanya juga turun,
sehingga E[max] praktis sama: 0.66304 vs 0.66309.

REKOMENDASI SAYA: pakai submission_v34_stabil.csv untuk slot final, bukan
ini. v35 disediakan karena diminta, dan karena bukti gabungannya sedikit
condong positif -- bukan karena terbukti lebih baik.

Total 25 ide diuji sepanjang pengembangan; 23 rugi atau nol, 2 (z-score dan
listwise ini) tidak lolos konfirmasi.

KELUARAN:
  submission_v35_lnet.csv    META v26 + listwise, 24 seed
  submission_v35_v26.csv     META v26 murni, 24 seed (pembanding, = v34)
